...
Manual `scp`, manual `rsync`, and direct server-side deployment are no longer part of this project's release or rollback process.
...
手动 `scp`、手动 `rsync` 和直接服务器部署不再属于本项目的发布或回退流程。
