...
import { articlePath, getPosts, getSiteCopy, getTerms, localizedPath, taxonomyPath, type Locale } from "@/lib/content";
  ...
  return (
    <AppShell locale={locale} alternatePath={alternate}>
      ...
      <section className="section topic-section" data-reveal>
        ...
        <nav className="topic-grid" aria-label={copy.categories}>{categories.map((category, index) => <Link href={taxonomyPath(locale, "categories", category.slug)} key={category.slug}><span>0{index + 1}</span><strong>{category.name}</strong><small>{category.count} {copy.articles}</small><i aria-hidden="true">↗</i></Link>)}</nav>
      </section>
      ...
      <section id="services" className="section service-section" data-reveal>
      ...
      <section id="contact" className="contact-section section-grid" data-reveal>
