...
categories: ["Web 工程"]
description: "一次边缘站点发布故障：普通 HTTP smoke 通过，真实浏览器仍返回 500，最终通过显式绑定检测和安全回退消除运行时错误。"
---

一个新版边缘站点通过了构建、单元测试和多轮普通 HTTP smoke，但真实浏览器首次打开 HTML 页面时仍返回 500。继续用简单请求验证时大多正常，带浏览器请求头的导航却能稳定复现失败。
...
实现把“平台通常提供某项能力”当成了“本次部署一定注入该绑定”。普通 smoke 没有命中相关分支，所以发布前检查未发现问题；真实 HTML 与图片请求进入这些分支后，未捕获的运行时异常直接变成 500。
...
- 只有检测到显式 HTML 缓存绑定时，才执行缓存读取和写入；否则直接走无缓存渲染。
...
缓存、图片优化和可观测性通常是增强能力，不应成为 HTML 可用性的单点依赖。边缘代码应把运行时绑定视为显式能力契约：存在才使用，缺失则进入经过测试的安全路径。
