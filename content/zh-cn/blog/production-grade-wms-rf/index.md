...
categories: ["系统设计"]
description: "一次 WMS RF 全量迁移实践：移动身份、仓库范围、复用领域状态机、弱网恢复和原生网络打印。"
---
...
验证覆盖后端模块测试、移动端单元测试、浏览器 smoke、TypeScript 构建、Capacitor 同步、Android 单测和 Debug APK。还增加迁移矩阵、权限隐私说明、并行切换手册以及专用 smoke 工具。

软件门禁全部通过，但真实 RF 与打印机仍被明确列为现场验收项。模拟和自动化可以证明接口、状态和构建正确，却不能替代射频设备的扫描速度、网络质量和打印效果。

生产级 RF 的关键不是页面数量，而是任何一次扫描和确认都可追踪、可重放但不重复，并且永远不能越过仓库边界。
