...
For a personal technical blog, this is not just a small configuration issue. It blocks all publishing, breaks trust, and makes every later content update useless until the access path is fixed.
...
This gives the site a clean operating model: write content, build Hugo, deploy static files, and let Nginx serve them efficiently.

## What I learned

The key lesson is that deployment problems should be handled from the outside in:
